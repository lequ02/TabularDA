...
# Focused comparison requested: joint generators versus RF/XGBoost targets.
source(file.path(root, "rf_xgb_analysis.R"), local = TRUE)

# Paper comparison and a clearly separate borrowed-score sensitivity analysis.
source(file.path(root, "paper_comparison.R"), local = TRUE)
...
cat("\nSession:\n"); print(sessionInfo())
sink()
cat("Saved tables, 27 PNG/PDF figure pairs, paper comparison and hypothetical sensitivity, legacy HTML report, and revised abstract to", root, "\n")
