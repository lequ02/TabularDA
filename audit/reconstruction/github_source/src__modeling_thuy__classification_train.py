...
from models_folder import model_mnist12, model_mnist28, model_intrusion, model_adult, model_census_kdd,model_census,model_covertype,model_credit
...
class train:
    ...
    def setup_data(self, dataset_name, train_option, augment_option, mix_ratio, n_sample, test_option, validation, batch_size):
        ...
        if dataset_name.lower() in ['mnist12', 'mnist28', 'intrusion', 'covertype']: #more than 2 classes
            self.multi_y = True
        elif dataset_name.lower() in ['adult', 'census_kdd', 'credit', 'census']: #binary
            self.multi_y = False
    ...
    def setup_trainer(self, pre_trained_w_file):
        ...
        if self.dataset_name.lower() == "adult":
        ...
        elif self.dataset_name.lower() == "census_kdd":
            if self.train_option.lower() == "mix"  :
                model = model_census_kdd.DNN_Census_Kdd_mix(input_size=input_size).to(device)
                self.model_name = "DNN_Census_Kdd_mix"
            else:
                model = model_census_kdd.DNN_Census_Kdd(input_size=input_size).to(device)
                self.model_name = "DNN_Census_Kdd"
            criterion = nn.BCELoss()
            ...
            model = model_covertype.DNN_Covertype(input_size=input_size).to(device)
