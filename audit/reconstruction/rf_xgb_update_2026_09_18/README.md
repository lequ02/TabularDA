...
Required R packages: ggplot2, lme4, lmerTest, emmeans, car and MASS. An optional first argument supplies another input directory containing the same two CSVs. Outputs are replaced only within this analysis folder. `analyse.R` automatically runs the readable companions `legacy_artifacts.R` and `rf_xgb_analysis.R`.
