...
save_four_panel_comparison <- function(root) {
  ...
  panels <- c("CTGAN-based", "TVAE-based", "Feature generator input", "End-to-end vs hybrid")
  panel_order <- panels[c(4, 3, 1, 2)]
  panel_labels <- setNames(c("A. End-to-end generative synthesis vs. FFTS",
                            "B. FFTS: target inclusion in generator training",
                            "C. CTGAN: end-to-end vs. FFTS",
                            "D. TVAE: end-to-end vs. FFTS"), panel_order)
  ...
  top <- data.frame(panel = ifelse(scores$tvae == 0, panels[1], panels[2]),
                    ...
                    unit = "Task/configuration", generator = ifelse(scores$tvae == 0, "CTGAN", "TVAE"))
  ...
  inclusion <- aggregate(macro_max ~ dataset + tvae + has_y, hybrid, mean)
  bottom_left <- data.frame(panel = panels[3], dataset = inclusion$dataset,
                           ...
                           score = inclusion$macro_max, approach = "hybrid", unit = "Task/generator mean",
                           generator = ifelse(inclusion$tvae == 0, "CTGAN", "TVAE"))
  families <- aggregate(macro_max ~ dataset + tvae + approach, scores, mean)
  bottom_right <- data.frame(panel = panels[4], dataset = families$dataset,
                            ...
                            unit = "Task/generator mean", generator = ifelse(families$tvae == 0, "CTGAN", "TVAE"))
  ...
  stopifnot(nrow(observations) == 72, all(table(observations$panel) == c(16, 16, 20, 20)),
            all(table(bottom_left$x) == 8), all(table(bottom_right$x) == 8),
            !anyDuplicated(bottom_left[c("dataset", "generator", "x")]),
            !anyDuplicated(bottom_right[c("dataset", "generator", "x")]))
  ...
  figure <- ggplot(observations, aes(x, score)) +
    ...
    scale_x_discrete(labels = function(labels) {
      labels[labels == "Target omitted from generator training"] <- "Target excluded"
      labels[labels == "Target included in generator training"] <- "Target included"
      labels[labels == "hybrid"] <- "FFTS"
      labels
    }) +
    scale_fill_manual(values = colors, labels = c(e2e = "End-to-end synthesis", hybrid = "FFTS (proposed)")) +
    scale_color_manual(values = colors, labels = c(e2e = "End-to-end synthesis", hybrid = "FFTS (proposed)")) +
    scale_y_continuous(limits = c(.3, 1), breaks = seq(.3, 1, .1)) +
    labs(title = "Factorized feature\u2013target synthesis (FFTS)\nvs. end-to-end generative synthesis",
         x = NULL, y = "downstream macro-F1", fill = NULL, color = NULL) +
  ...
  # Repeat the shared y-axis title once per panel row, retaining the fixed scale.
  figure <- ggplotGrob(figure)
  y_title <- which(figure$layout$name == "ylab-l")
  y_column <- figure$layout[y_title, c("l", "r")]
  figure$grobs[[y_title]] <- grid::nullGrob()
  panel_rows <- sort(unique(figure$layout$t[grepl("^panel-", figure$layout$name)]))
  stopifnot(length(y_title) == 1, length(panel_rows) == 2)
  for (panel_row in panel_rows) {
    figure <- gtable::gtable_add_grob(figure,
      grid::textGrob("downstream macro-F1", rot = 90,
                     gp = grid::gpar(fontsize = 13, col = "black")),
      t = panel_row, l = y_column$l, r = y_column$r,
      clip = "off", name = paste0("row-y-title-", panel_row))
  }
