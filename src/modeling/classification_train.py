...
class train:
    ...
    def setup_data(self, dataset_name, train_option, augment_option, mix_ratio, n_sample, test_option, validation, batch_size):
        ...
        if dataset_name.lower() in ['mnist12', 'mnist28', 'intrusion', 'covertype']: #more than 2 classes
            self.multi_y = True
        elif dataset_name.lower() in ['adult', 'census', 'census_kdd', 'credit']: #binary
            self.multi_y = False
        else:
        ...
        if test_option != 'original':
            raise ValueError("Corrected evaluation requires the original real test set")
        self.batch_size = batch_size
        self.validation = validation

        if dataset_name.lower() == "mnist28" or dataset_name.lower() == "mnist12":
            self.data_loader = DataLoaderMNIST(self.dataset_name, self.batch_size, seed=self.seed)
            ...
            self.data_loader = data_loader(self.dataset_name, self.batch_size, multi_y = self.multi_y, problem_type = 'classification', seed=self.seed)
            self.train_data, self.dev_data = self.data_loader.load_train_augment_data(self.train_option, self.augment_option, self.mix_ratio, self.n_sample, self.validation)
    ...
    def setup_trainer(self, pre_trained_w_file):
        ...
        if self.dataset_name.lower() in {"adult", "credit"}:
            model = model_adult.DNN_Adult(input_size=input_size).to(device)
            self.model_name = "DNN_Adult"
            criterion = nn.BCELoss()

        elif self.dataset_name.lower() in {"census", "census_kdd"}:
        ...
        #     criterion = nn.CrossEntropyLoss()
            ...
            model = model_mnist28.DNN_MNIST28(input_size=input_size, output_size=self.data_loader.num_classes).to(device)
            self.model_name = "DNN_MNIST28"
            criterion = nn.CrossEntropyLoss()
        elif self.dataset_name.lower() == "mnist12":
            model = model_mnist12.DNN_MNIST12(input_size=input_size, output_size=self.data_loader.num_classes).to(device)
            self.model_name = "DNN_MNIST12"
            criterion = nn.CrossEntropyLoss()
        ...
        if pre_trained_w_file is not None:
            ...
            model.load_state_dict(torch.load(pre_trained_w_file, weights_only=True))
    ...
    def training(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.patience == -1: # select the final epoch when early stopping is disabled
        ...
        write_run_record(
            ...
            synthetic_label_counts=self.data_loader.synthetic_label_counts,
            split_manifest_path=self.data_loader.manifest_path,
            classifier=self.model_name, batch_size=self.batch_size,
            learning_rate=self.learning_rate, epoch_budget=self.num_epochs,
...
#        self.plot_loss_and_f1_curves(train_losses, dev_losses, train_scores[self.metric_to_plot], dev_scores[self.metric_to_plot])
    ...
    def check_early_stop(self, loss, score, best_loss, best_score, patience_counter):
        ...
        if self.early_stop_criterion == "loss":
            if best_loss > loss + tolerance:
                best_loss = loss
                self.save_model(self.w_file_name)
                self.selected_epoch = self.current_epoch
                patience_counter = 0
            else:
                patience_counter += 1
    ...
    def validate(self, data, load_weight=False):
        ...
        if load_weight:
            self.trainer.model.load_state_dict(torch.load(self.w_dir + self.w_file_name, weights_only=True))
        ...
        with torch.no_grad():
            for batch_idx, (X, y) in enumerate(data):
                ...
                if self.multi_y == True:
                    ...
                    all_scores.extend(output[:, 0].cpu().numpy())

                loss += self.trainer.criterion(output, y).item() * len(y)

                all_preds.extend(predicted.cpu().numpy().ravel())
    ...
    def compute_scores(self, y, y_hat, y_score=None):
