...
class train:
    ...
    def setup_data(self, dataset_name, train_option, augment_option, mix_ratio, n_sample, test_option, validation, batch_size):
        ...
        self.data_loader = data_loader(self.dataset_name, self.batch_size, multi_y = self.multi_y, problem_type = 'regression', seed=self.seed)
    ...
    def setup_eval_metric(self, eval_metrics, metric_to_plot):
        ...
        self.eval_metrics = sorted(m.lower() for m in eval_metrics)
    ...
    def check_early_stop(self, loss, score, best_error, best_score, patience_counter):
        ...
        if self.early_stop_criterion in ["mae", "mse", "rmse", "mape", "loss"]:
            ...
            raise ValueError(f"Cannot recognize the metric for early stopping {self.early_stop_criterion}")
    ...
    def compute_scores(self, y, y_hat):
        ...
        for metric in self.eval_metrics:
            if metric == "mse":
                ...
                raise ValueError(f"Cannot recognize the metric {metric}")
        return ret_score
