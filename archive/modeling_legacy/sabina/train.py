...
class train:
    ...
    def __init__(self, dataset_name, batch_size, learning_rate, 
        ...
        if early_stop_criterion not in {'loss', 'f1'}:
            raise ValueError(f"Unsupported early-stop criterion: {early_stop_criterion}")
        if dataset_name.lower() == 'news' and early_stop_criterion != 'loss':
            raise ValueError("News regression requires loss-based checkpoint selection")
        ...
        self.train_data, self.dev_data = self.data_loader.load_train_augment_data(self.train_option, self.augment_option, validation=0.2)
    ...
    def setup_trainer(self, pre_trained_w_file):
        ...
        if self.dataset_name.lower() == "adult":
            ...
            model = model_intrusion.DNN_Intrusion(input_size=input_size, output_size=7).to(device)
        ...
        if pre_trained_w_file is not None:
            ...
            model.load_state_dict(torch.load(self.w_dir + pre_trained_w_file, weights_only=True))
    ...
    def training(self):
        ...
        with open(self.acc_dir + self.acc_file_name, 'w') as acc_file:
            if self.dataset_name.lower() in ["adult", "census"]:
                acc_file.write("global_round,train_loss,train_acc,train_f1,dev_loss,dev_acc,dev_f1\n")
            else:
                acc_file.write("global_round,train_mse,train_r2,dev_mse,dev_r2\n")
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.dataset_name.lower() in ["adult", "census", 'mnist12', 'mnist28', 'intrusion']:
                ...
                test_loss, test_acc, test_f1 = self.validate_classification(data=self.dev_data)
                ...
                test_loss, test_mse, test_r2 = self.validate_regression(data=self.dev_data)
                test_losses.append(test_loss)
        ...
        if self.dataset_name.lower() == "news":
            self.validate_regression(data=self.test_data)
        else:
            self.validate_classification(data=self.test_data)
    ...
    def validate_classification(self, data=None, load_weight=False):
    ...
    def validate_regression(self, data=None, load_weight=False):
        print("Validation statistic...")
        ...
        with torch.no_grad():
            for X, y in data:
                X, y = X.to(device), y.to(device).float().reshape(-1, 1)
                output = self.trainer.model(X)
                total_loss += self.trainer.criterion(output, y).item() * len(y)
                total += len(y)
