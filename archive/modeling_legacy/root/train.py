...
class train:
  ...
  def training(self):
      ...
      for epoch in range(self.num_epochs):
          ...
          if self.dataset_name.lower() in ["adult", "census"]:
              ...
              test_loss, test_acc, test_f1 = self.validate_classification(data=self.data.dev_data)
              ...
              test_loss, test_mse, test_r2 = self.validate_regression(data=self.data.dev_data)
              test_losses.append(test_loss)
      ...
      self.trainer.model['model'].load_state_dict(
          torch.load(self.w_dir + self.w_file_name, weights_only=True))
      self.data.load_test_data()
      if self.dataset_name.lower() in ["adult", "census"]:
          self.validate_classification(data=self.data.test_data)
      else:
          self.validate_regression(data=self.data.test_data)
  ...
  def validate_classification(self, data=None, load_weight=False):
      ...
      if load_weight:
          self.trainer.model['model'].load_state_dict(torch.load(self.w_dir + self.w_file_name, weights_only=True))
