...
class data_loader:
    ...
    def load_test_data(self):
        ...
        return self._load_data_in_batches(test_df)

    def _load_data_in_batches(self, df, is_test=False):
        

        y = df[self.paths['target_name']]
        X = df.drop(self.paths['target_name'], axis=1)
        X = self._standardize(X)
        return self._distribute_in_batches(X.values, y.values, is_test)
        
    def _standardize(self, df):
        scaler = StandardScaler()
        return pd.DataFrame(scaler.fit_transform(df), columns=df.columns)
        
    def _distribute_in_batches(self, X, y, is_test=False):
        print("Type X", type(X))
        print("Type y", type(y))
        ...
        return DataLoader(ConcatDataset(batches), batch_size=batch_size)
